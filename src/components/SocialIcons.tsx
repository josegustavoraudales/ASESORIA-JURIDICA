...
const socials = [
  { url: siteConfig.whatsappUrl, icon: WhatsAppIcon, label: "WhatsApp", isCustom: true },
  { url: siteConfig.instagramUrl, icon: Instagram, label: "Instagram" },
  { url: siteConfig.facebookUrl, icon: Facebook, label: "Facebook" },
  { url: siteConfig.tiktokUrl, icon: TikTokIcon, label: "TikTok", isCustom: true },
];

export const SocialIcons = ({ size = 18, className = "", showLabels = false }: Props) => (
    ...
    {socials
      ...
      .map((s) => (
        <a
          ...
          aria-label={s.label}
        >
          {s.isCustom ? <s.icon size={size} /> : s.icon && <s.icon size={size} />}
          {showLabels && <span className="text-sm">{s.label}</span>}
        </a>
      ))}
  </div>
);
